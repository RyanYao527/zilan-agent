...
def _runtime_evidence_rel_path_from_ref(ref: str) -> str | None:
...
def _runtime_evidence_refs_from_text(text: str) -> set[str]:
    refs: set[str] = set()
    for match in RUNTIME_EVIDENCE_FILE_REF_RE.finditer(text):
        rel_path = _runtime_evidence_rel_path_from_ref(match.group(1))
...
def _runtime_evidence_summary_only_refs(index_text: str) -> set[str]:
    summary_section = _markdown_section(index_text, "Summary-Only Runtime Evidence")
    return _runtime_evidence_refs_from_text(summary_section)


def _check_runtime_evidence_index_references(root: Path, index_text: str, failures: list[str]) -> None:
    for rel_path in sorted(_runtime_evidence_refs_from_text(index_text)):
...
def _check_runtime_evidence_batch_manifests(
    root: Path,
    failures: list[str],
    *,
    ...
    for batch_path in sorted(evidence_dir.glob("*batch.yaml")):
        ...
        data = _load_yaml(root, rel_path, failures, [], strict_yaml=True)
        if data is None:
            continue
        if not isinstance(data, dict):
...
def _check_runtime_evidence_docs(root: Path, failures: list[str]) -> None:
    ...
    _check_runtime_evidence_index_references(root, nav_index_text, failures)
    _check_runtime_evidence_batch_manifests(
        root,
        failures,
        summary_only_refs=_runtime_evidence_summary_only_refs(nav_index_text),
    )
    check_required_fragments(
        clean_install_text,
        (
