...
The current non-OpenAI provider triage is in `docs/provider-routes.md`. Volcengine OpenAI-compatible is `tested` for one ZC-02 live run through `chat-completions`; DeepSeek, GLM, and Qwen remain `config-only` because this repository has provider metadata but no native dry-run/live harnesses or dated runtime evidence for those routes.
