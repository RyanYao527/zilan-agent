...
Volcengine OpenAI-compatible is `tested` for ZC-01 through ZC-03 live runs through `chat-completions`; native OpenAI API remains `harness-ready`. DeepSeek, GLM, and Qwen currently remain provider metadata routes unless a native harness, credentials, and dated runtime evidence are added. See `docs/provider-routes.md` for the current route triage.
