...
| Reasoning contract runner | `scripts/reasoning_contract_runner.py` combines the semantic retrieval dry run, role coverage, answer-contract review, Hetuvidya validator, and cognitive-analysis mapper into one fixture-only local entrypoint. Its `pass` / `fail` / `review_needed` status is a contract-review status, not platform validation or doctrinal grading. |
