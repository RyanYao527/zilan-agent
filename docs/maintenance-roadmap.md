...
| Agama evidence checker prototype | `scripts/agama_evidence_checker.py` reads only structured `tests/reasoning_cases.yaml` Agama evidence fixtures and emits deterministic JSON/text checks for citation requirements, representative-search scope, local `context/agama/` references, and collation boundaries. It intentionally does not run Agama search, perform CBETA XML collation, call providers, alter prompts, or grade citation correctness. |
