...
| P1 | Semantic retrieval | Define an answer-level sample review for `SRQ-02` only after the expected reasoning-error output shape is clear. | Committed samples can check whether answers identify `因不成` without introducing provider calls or an LLM judge. |
