...
| P1 | Semantic retrieval | Use `SRQ-02` through `SRQ-04` contracts in post-contract runtime review before adding more fixture-level answer contracts. | Runtime review records which committed contracts real answers satisfy or miss, without introducing provider calls, an LLM judge, or platform-status changes. |
