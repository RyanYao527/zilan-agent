...
| P1 | Reasoning quality | Run transcript-backed `SRQ-02`, `SRQ-03`, `SRQ-04`, and `ZC-05` target prompts on one tested route before changing prompts. | Compact answer excerpts are committed and checked with `semantic_answer_contract_review.py --answer-file`, distinguishing evidence gaps from answer-quality gaps. |
| P1 | Semantic retrieval | Use the `SRQ-04` Agama citation contract against transcript-backed answer excerpts before adding more fixture-level retrieval contracts. | Runtime evidence shows whether answers include CBETA/local anchors, representative search-scope language, and collation boundaries without provider-status changes. |
