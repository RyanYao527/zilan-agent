...
| P1 | Semantic retrieval | Generate fixture candidates from `search_agama.py --json` while preserving `citation` and `passage_citation`. | Query fixtures can be refreshed from the keyword baseline without hand-maintaining all Agama passage fields, while avoiding embeddings or vector storage. |
