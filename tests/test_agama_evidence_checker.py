...
def test_agama_evidence_checker_maps_zr05_citation_boundary() -> None:
    ...
    assert result["mode"] == "agama-evidence-checker-v0"
    assert result["output_schema"] == "agama-evidence-checker-output-v0"
...
def test_agama_evidence_checker_cli_json_output_is_machine_readable() -> None:
    ...
    assert data["mode"] == "agama-evidence-checker-v0"
    assert data["case_id"] == "ZR-05"
    assert data["evidence_reviews"][0]["agama_evidence"]["citation_required"]["status"] == "required"
